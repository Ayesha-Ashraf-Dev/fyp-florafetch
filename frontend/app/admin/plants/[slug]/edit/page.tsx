...
import {
  ...
  HUMIDITY_OPTIONS, GROWTH_OPTIONS, labelStyle, sectionStyle, sectionTitleStyle,
...
export default function EditPlantPage() {
  ...
  const loadPlant = async () => {
    try {
      setPageLoading(true);
      const plant = await apiClient.getPlant(plantSlug);
      setPlantIdentifier(plant.slug || String(plant.id));
      setFormData({
        name: plant.name,
        ...
        sunlight_requirement: plant.sunlight_requirement || "moderate",
        watering_frequency: plant.watering_frequency || "moderate",
        soil_type: plant.soil_type || "potting soil",
        temperature_min: String(plant.temperature_min ?? 15),
        temperature_max: String(plant.temperature_max ?? 25),
        humidity_level: plant.humidity_level || "moderate",
        is_pet_friendly: plant.is_pet_friendly || false,
        is_low_maintenance: plant.is_low_maintenance || false,
        growth_rate: plant.growth_rate || "slow",
      });
      const urls: string[] = [];
      ...
      if (plant.images) {
        try {
          ...
          if (Array.isArray(parsed)) parsed.forEach((u: string) => { if (u && !urls.includes(u)) urls.push(u); });
        } catch { /* ignore */ }
      }
      setImageUrls(urls);
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const loadingId = toast.loading("Updating plant...");
    ...
    try {
      ...
      await apiClient.updatePlant(plantIdentifier, payload);
